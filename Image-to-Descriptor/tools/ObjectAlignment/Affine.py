...
# --- Helper Function to save Affine output to JSON ---
def save_affine_output_to_json(output_path, M, inlier_mask,
                                  image1_path, image2_path, num_good_matches, num_inliers,
                                  ransac_threshold, max_iters, confidence):
    """
    Saves the computed Affine matrix and inlier mask to a JSON file.
    """
    # Ensure M is handled even if None
    M_list = M.tolist() if M is not None else None
    ...
    json_data = {
        "alignment_tool": "Affine_RANSAC", # Changed from Homography
        "tool_version": {
        ...
        "affine_parameters_used": { # Changed from homography_parameters_used
        ...
        "affine_results": { # Changed from homography_results
            "affine_matrix": M_list, # Changed from homography_matrix
            "num_inliers": num_inliers_python,
    ...
    try:
        ...
        print(f"[INFO] Affine results saved to: {output_path}")
    except IOError as e:
        print(f"[ERROR] Failed to save Affine JSON: {e}")
...
# Folder for Affine output
output_dir = "/Users/pop/Desktop/project_n2n/affine_outputs" # Changed output folder
os.makedirs(output_dir, exist_ok=True)

# Affine Parameters (can be tuned) - ransacReprojThreshold might need different tuning than Homography
...
# --- Main Process ---
print("--- Starting Affine Transformation Estimation with RANSAC ---")
...
if len(kp1) == 0 or len(kp2) == 0:
    print("[ERROR] No keypoints loaded for one or both images. Cannot proceed with Affine Estimation.")
    sys.exit(1)
...
if len(good_matches_data) == 0:
    print("[ERROR] No good matches found. Cannot compute Affine Transformation.")
    sys.exit(1)
...
# Minimum 3 points for Affine Transformation (compared to 4 for Homography)
if pts_src.shape[0] < 3:
    print(f"[ERROR] Not enough (minimum 3) good matches ({pts_src.shape[0]} found) to compute Affine Transformation. Exiting.")
    sys.exit(1)

# 3. Compute Affine Transformation Matrix using RANSAC
# cv2.estimateAffine2D directly supports RANSAC
print(f"\n[INFO] Computing Affine Transformation using RANSAC with threshold: {RANSAC_REPROJ_THRESHOLD} pixels...")
M, mask = cv2.estimateAffine2D(
    pts_src,
    pts_dst,
    method=cv2.RANSAC, # Explicitly use RANSAC
...
if mask is not None:
    ...
    print(f"[INFO] Affine Transformation computed. Found {num_inliers} inliers out of {num_good_matches_input} good matches.")
    if M is None:
        print("[WARNING] Affine matrix is None. This can happen if RANSAC fails to find a good model.")
    elif num_inliers == 0:
        print("[WARNING] No inliers found by RANSAC. Affine Transformation might be unreliable.")
else:
    print("[WARNING] Mask is None. This indicates an issue with Affine Transformation computation.")


# 4. Save Results to JSON
img1_base = os.path.basename(img1_original_path).split('.')[0]
img2_base = os.path.basename(img2_original_path).split('.')[0]
output_json_name = f"{img1_base}_vs_{img2_base}_affine_output.json" # Changed output filename
output_json_path = os.path.join(output_dir, output_json_name)

save_affine_output_to_json( # Changed function call
    output_json_path, M, mask,
...
try:
    ...
    if M is not None and num_inliers > 0:
        ...
        # Warp image 1 using Affine Transformation
        h_target, w_target = img2_orig_color.shape[:2]
        # For Affine, warpAffine expects a (2,3) matrix, and the output size.
        img1_warped = cv2.warpAffine(img1_orig_color, M, (w_target, h_target)) # Changed warpPerspective to warpAffine
        ...
        plt.title(f"Image 1 Warped (Affine) onto Image 2 (Blended, alpha={alpha})") # Title change
        plt.axis('off')
        ...
        print("[WARNING] Skipping visualization: Affine matrix is None or no inliers found.")
...
print("\n--- Affine Transformation Estimation with RANSAC Completed ---")