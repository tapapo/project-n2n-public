...
const BFMatcherNode = memo(({ id, data, selected }: NodeProps<CustomNodeData>) => {
  ...
  // ✅ แก้ไข: ไม่เปลี่ยนเป็นสีเขียว/แดง ให้คงสีส้มไว้
  ...
  // ตัด else if (isSuccess) ออก เพื่อให้กลับไปใช้ default (ส้ม)
      ...
      <div className="bg-gray-700 text-orange-400 rounded-t-xl px-2 py-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onRun}
            disabled={isRunning}
            // ✅ แก้ไข: ปุ่มเป็นสีส้มเสมอ (เปลี่ยนแค่ตอน Running เป็นเหลือง)
            className={[
