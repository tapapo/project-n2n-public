...
const SurfNode = memo(({ data }: NodeProps<CustomNodeData>) => {
  const resultUrl =
    (data?.payload && (data.payload.result_image_url as string)) ||
    (data?.payload && (data.payload.surf_vis_url as string)) ||
    undefined;
  ...
  return (
    <div className="bg-gray-800 border-2 border-teal-500 rounded-xl shadow-2xl w-72 text-gray-200">
      ...
      <div className="bg-gray-700 text-center font-bold p-2 text-teal-400 rounded-t-xl">SURF</div>

      <div className="p-4 space-y-3">
        {resultUrl && <img src={resultUrl} alt="surf-result" className="w-full rounded-lg border border-gray-700 shadow-md object-contain max-h-56" draggable={false} />}
