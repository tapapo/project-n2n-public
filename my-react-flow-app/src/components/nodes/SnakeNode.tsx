...
// ✅ ตัด feature ออก เหลือแค่ 3 mode
...
type Params = {
  ...
  w_line: Numish;
  w_edge: Numish;
  max_iterations: Numish;
  gaussian_blur_ksize: Numish;
...
const DEFAULT_PARAMS: Params = {
  ...
  w_edge: 1.0,
  max_iterations: 250,
  gaussian_blur_ksize: 0, // 0 = no blur
...
// ------- utils แปลงค่าตัวเลขปลอดภัยตอน Save -------
const toInt = (v: Numish, fallback: number) => {
  const n = typeof v === 'string' && v.trim() === '' ? NaN : Number(v);
  return Number.isFinite(n) ? Math.trunc(n) : fallback;
};
const toFloat = (v: Numish, fallback: number) => {
  const n = typeof v === 'string' && v.trim() === '' ? NaN : Number(v);
...
// กัน Backspace/Delete ไปลบทั้ง node
const stopKeys: React.KeyboardEventHandler<HTMLInputElement | HTMLSelectElement> = (e) => {
  e.stopPropagation();
  const k = e.key;
  if (k === 'Backspace' || k === 'Delete' || k === 'Enter' || k === ' ') {
    // แค่นี้ก็พอให้ React Flow ไม่จับคีย์ node-level แล้ว
  }
};

// ====================================================================
// 💡 Helper Components (Num / Select)
// ====================================================================
...
const SnakeNode = memo(({ id, data }: NodeProps<CustomNodeData>) => {
  const rf = useReactFlow();
  const [open, setOpen] = useState(false);
  const [showAdv, setShowAdv] = useState(false);

  // อ่านค่าที่เคยตั้งไว้
  const savedParams = useMemo<Params>(() => {
    const p = ((data?.payload?.params || {}) as Partial<Params>) ?? {};
    return {
      ...DEFAULT_PARAMS,
      ...p,
      init_mode: normalizeInitMode((p as any).init_mode),
    };
  }, [data?.payload?.params]);
  ...
  const onSave = () => {
    // parse + sanitize เฉพาะตอนกด Save
    const next = { ...form };

    // CORE
    const alpha = toFloat(form.alpha, 0.2);
    const beta = toFloat(form.beta, 0.2);
    const gamma = toFloat(form.gamma, 0.1);
    const w_line = toFloat(form.w_line, 0.0);
    const w_edge = toFloat(form.w_edge, 1.0);
    ...
    const init_radius =
      form.init_radius === null || form.init_radius === '' ? null : Math.max(1, toInt(form.init_radius, 10));

    const from_point_x =
      form.from_point_x === null || form.from_point_x === '' ? null : toInt(form.from_point_x, 0);
    const from_point_y =
      form.from_point_y === null || form.from_point_y === '' ? null : toInt(form.from_point_y, 0);
  ...
  return (
    <div className="bg-gray-800 border-2 border-pink-500 rounded-xl shadow-2xl w-80 text-gray-200 overflow-visible">
      <Handle
        type="target"
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Num
                ...
                value={form.w_edge}
                step={0.05}
                onChange={(v) => setForm((s) => ({ ...s, w_edge: v }))}
              />
              <Num
                label="w_line"
                value={form.w_line}
                step={0.05}
                onChange={(v) => setForm((s) => ({ ...s, w_line: v }))}
              />
              <Num
              ...
              />
              <Num
                label="gaussian_blur_ksize (0=none)"
            ...
            {showAdv && (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    label="Init mode"
                    value={form.init_mode}
                    onChange={(v) => setForm((s) => ({ ...s, init_mode: v as InitMode }))}
                    options={[
                  ...
                  {form.init_mode === 'circle' && (
                    <div className="grid grid-cols-3 gap-2">
                      <Num
                        ...
                        value={form.init_cx}
                        step={1}
                        onChange={(v) => setForm((s) => ({ ...s, init_cx: v }))}
                      />
                      <Num
                        label="init_cy"
                        value={form.init_cy}
                        step={1}
                        onChange={(v) => setForm((s) => ({ ...s, init_cy: v }))}
                      />
                      <Num
                        label="init_radius"
                        value={form.init_radius}
                        min={1}
                        step={1}
                        onChange={(v) => setForm((s) => ({ ...s, init_radius: v }))}
                      />
                    </div>
                  )}
                  ...
                  {form.init_mode === 'point' && (
                    <div className="grid grid-cols-3 gap-2">
                      <Num
                        label="from_point_x"
                        value={form.from_point_x}
                        step={1}
                        onChange={(v) => setForm((s) => ({ ...s, from_point_x: v }))}
                      />
                      <Num
                        label="from_point_y"
                        value={form.from_point_y}
                        step={1}
                        onChange={(v) => setForm((s) => ({ ...s, from_point_y: v }))}
                      />
                      <Num
                        label="init_radius"
                        value={form.init_radius}
                  ...
                  {form.init_mode === 'bbox' && (
                    <div className="grid grid-cols-4 gap-2">
                      <Num
                        ...
                        value={form.bbox_x1}
                        step={1}
                        onChange={(v) => setForm((s) => ({ ...s, bbox_x1: v }))}
                      />
                      <Num
                        label="bbox_y1"
                        value={form.bbox_y1}
                        step={1}
                        onChange={(v) => setForm((s) => ({ ...s, bbox_y1: v }))}
                      />
                      <Num
                        label="bbox_x2"
                        value={form.bbox_x2}
                        step={1}
                        onChange={(v) => setForm((s) => ({ ...s, bbox_x2: v }))}
                      />
                      <Num
                        label="bbox_y2"
                        value={form.bbox_y2}
                        step={1}
                        onChange={(v) => setForm((s) => ({ ...s, bbox_y2: v }))}
                      />
                    </div>
                  )}
                </div>
