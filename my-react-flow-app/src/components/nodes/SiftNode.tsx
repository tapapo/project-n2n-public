...
const SiftNode = memo(({ id, data, selected }: NodeProps<CustomNodeData>) => {
  ...
  // ✅ FIXED: สีเขียวเสมอ (ยกเว้น Selected / Running)
  ...
  return (
    <div className={`bg-gray-800 border-2 rounded-xl shadow-2xl w-72 text-gray-200 overflow-visible transition-all duration-200 ${borderColor}`}>
      ...
      <div className="p-4 space-y-3">
        ...
        {resultUrl && (
          <img src={resultUrl} alt="sift-result" className="w-full rounded-lg border border-gray-700 shadow-md object-contain max-h-56" draggable={false} />
        )}
      ...
      <Modal open={open} title="SIFT Settings" onClose={handleClose}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <label className="text-xs text-gray-300">nOctaveLayers<input type="number" step={1} min={1} max={8} className="w-full mt-1 px-2 py-1 rounded bg-gray-900 border border-gray-700 text-gray-100" value={form.nOctaveLayers} onChange={(e) => { const v = Math.max(1, parseInt(e.target.value || '1', 10)); setForm((s: any) => ({ ...s, nOctaveLayers: v })); }} /></label>
          <label className="text-xs text-gray-300">contrastThreshold<input type="number" step="0.001" min={0} className="w-full mt-1 px-2 py-1 rounded bg-gray-900 border border-gray-700 text-gray-100" value={form.contrastThreshold} onChange={(e) => setForm((s: any) => ({ ...s, contrastThreshold: Number(e.target.value) }))} /></label>
          ...
          <label className="text-xs text-gray-300">sigma<input type="number" step="0.1" min={0} className="w-full mt-1 px-2 py-1 rounded bg-gray-900 border border-gray-700 text-gray-100" value={form.sigma} onChange={(e) => setForm((s: any) => ({ ...s, sigma: Number(e.target.value) }))} /></label>
        </div>
