...
import { Handle, Position, type NodeProps, useStore } from 'reactflow'; // ✅ เพิ่ม useStore
...
const BrisqueNode = memo(({ id, data, selected }: NodeProps<CustomNodeData>) => {
  ...
  // ✅ FIX: ใช้ useStore เช็คเส้นแบบ Real-time (แก้บั๊กลบเส้นแล้วจุดแดงไม่ขึ้น)
  ...
  if (selected) {
    borderColor = 'border-blue-400 ring-2 ring-blue-500'; // Selected
  } else if (isRunning) {
    borderColor = 'border-yellow-500 ring-2 ring-yellow-500/50'; // Running
  }

  // ✅ Handle Class Logic
  // Input (Left): แดงถ้าพังและไม่มีสาย
