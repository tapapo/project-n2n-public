    ...
    <div className={`bg-gray-800 border-2 rounded-xl shadow-2xl w-72 text-gray-200 overflow-visible transition-all duration-200 ${
      ...
      <div className="bg-gray-700 text-red-400 rounded-t-xl px-2 py-2 flex items-center justify-between font-bold">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={isRunning}
            className={`px-2 py-1 rounded text-xs font-semibold transition-colors duration-200 text-white cursor-pointer ${isRunning ? 'bg-yellow-600' : 'bg-red-600 hover:bg-red-700'}`}
          >
      ...
      <Modal open={open} title="Real-ESRGAN Settings" onClose={handleClose}>
        <div className="grid grid-cols-1 gap-3 text-xs text-gray-300">
          <div>
            <label className="text-gray-500 uppercase text-[9px] font-bold">Model Weights</label>
            <div className="w-full bg-gray-900 rounded border border-gray-700 p-2 mt-1 text-red-400 font-mono opacity-80 select-none">
              {FIXED_MODEL}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mt-1">
            <label>Scale 
              <input type="number" step="0.5" className="w-full bg-gray-900 rounded border border-gray-700 p-1 mt-1 outline-none focus:border-red-500" value={form.scale} onChange={e => setForm((s: Params) => ({ ...s, scale: Number(e.target.value) }))} />
            </label>
            <label>Denoise 
              <input type="number" step="0.1" className="w-full bg-gray-900 rounded border border-gray-700 p-1 mt-1 outline-none focus:border-red-500" value={form.denoise} onChange={e => setForm((s: Params) => ({ ...s, denoise: Number(e.target.value) }))} />
            </label>
          </div>
        </div>
        <div className="flex justify-end gap-2 pt-4">
          <button onClick={handleClose} className="px-3 py-1 rounded bg-gray-700 text-xs cursor-pointer hover:bg-gray-600">Cancel</button>
          <button onClick={onSave} className="px-3 py-1 rounded bg-red-600 text-white text-xs font-bold cursor-pointer hover:bg-red-500">Save</button>
        </div>
      </Modal>
    </div>
  )
