...
type Params = { gaussian_blur: boolean; blur_ksize: number; invert: boolean; };
const DEFAULT_PARAMS: Params = { gaussian_blur: false, blur_ksize: 5, invert: false };
...
const OtsuNode = memo(({ id, data, selected }: NodeProps<CustomNodeData>) => {
  ...
  const onSave = () => {
    ...
    rf.setNodes(nds => nds.map(n => n.id === id ? { ...n, data: { ...n.data, payload: { ...(n.data?.payload || {}), params: { ...form, blur_ksize: oddK } } } } : n));
    setOpen(false);
  };

  const resp = data?.payload?.json as any | undefined;
  const previewUrl: string | undefined = (data?.payload?.result_image_url as string | undefined) || (data?.payload?.preview_url as string | undefined) || (resp?.binary_url as string | undefined);
    ...
    <div className={`bg-gray-800 border-2 rounded-xl shadow-2xl w-72 text-gray-200 overflow-visible transition-all duration-200 ${borderColor}`}>
      ...
      <div className="bg-gray-700 text-pink-400 rounded-t-xl px-3 py-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setOpen(true)} className="h-5 w-5 rounded-full bg-white flex items-center justify-center shadow ring-2 ring-gray-500/60 hover:ring-gray-500/80">
      ...
      <div className="p-4 space-y-3">
        <p className="text-sm text-gray-300">{previewUrl ? `Threshold = ${thr ?? '?'}` : 'Connect Image Input and run'}</p>
        {previewUrl && <img src={abs(previewUrl)} alt="otsu" className="w-full rounded-lg border border-gray-700 shadow-md object-contain max-h-56" draggable={false} />}
      </div>
      ...
      <Modal open={open} title="Otsu Settings" onClose={onClose}>
        <div className="space-y-3 text-xs text-gray-300">
          <label className="flex items-center gap-2"><input type="checkbox" checked={form.gaussian_blur} onChange={(e) => setForm((s) => ({ ...s, gaussian_blur: e.target.checked }))} />Gaussian blur before threshold</label>
          <label className="block">Blur kernel size (odd)<input type="number" min={3} step={2} className="w-full mt-1 px-2 py-1 rounded bg-gray-900 border border-gray-700 text-gray-100" value={form.blur_ksize} onChange={(e) => setForm((s) => ({ ...s, blur_ksize: Number(e.target.value) }))} /></label>
          <label className="flex items-center gap-2"><input type="checkbox" checked={form.invert} onChange={(e) => setForm((s) => ({ ...s, invert: e.target.checked }))} />Invert output</label>
          <div className="flex justify-end gap-2 pt-3"><button onClick={onClose} className="px-3 py-1 rounded bg-gray-700 text-gray-200 hover:bg-gray-600">Close</button><button onClick={onSave} className="px-3 py-1 rounded bg-pink-600 text-white hover:bg-pink-700">Save</button></div>
        </div>
      </Modal>
    </div>
  );
