...
const Sidebar = ({ onLoadTemplate }: SidebarProps) => {
  ...
  const jobs = useMemo(() => ([
    ...
    { name: 'Classification', headerColor: 'text-pink-400', algorithms: [{ type: 'otsu', label: "Otsu's Threshold", color: 'bg-pink-600' }, { type: 'snake', label: 'Snake (Active Contour)', color: 'bg-pink-600' }] },
    { name: 'Quality Assessment', headerColor: 'text-blue-400', algorithms: [{ type: 'brisque', label: 'BRISQUE', color: 'bg-blue-600' }, { type: 'psnr', label: 'PSNR', color: 'bg-blue-600' }, { type: 'ssim', label: 'SSIM', color: 'bg-blue-600' }] },
    ...
    <>
      <aside className={['border-r border-gray-800 bg-gray-900 h-full shadow-2xl flex flex-col transition-all duration-300 z-20', collapsed ? 'w-14' : 'w-72'].join(' ')}>
        ...
        {!collapsed && (
          <div className="flex p-2 gap-1 border-b border-gray-800">
            ...
            <button onClick={() => setActiveTab('templates')} className={`flex-1 py-1.5 text-xs font-bold rounded-md flex items-center justify-center gap-1 ${activeTab === 'templates' ? 'bg-teal-600/20 text-teal-400 border border-teal-500/50' : 'text-gray-500 hover:bg-gray-800'}`}>
        ...
        {!collapsed ? (
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            <div className="px-1 pb-2 flex gap-2 mb-1">
              <button onClick={expandAll} className="flex-1 text-[10px] px-2 py-1 rounded bg-gray-800 text-gray-500 hover:text-white hover:bg-gray-700">EXPAND ALL</button>
            ...
            {activeTab === 'nodes' && jobs.map(job => {
                ...
                return (
                  <div key={job.name} className="rounded-lg overflow-hidden border border-gray-800/50 bg-gray-800/30 mb-2">
                    <div className={`flex items-center justify-between p-2.5 cursor-pointer ${isOpen ? 'bg-gray-800 text-gray-200' : 'hover:bg-gray-800 text-gray-400'}`} onClick={() => toggleNodeGroup(job.name)}>
                      <span className={`text-xs font-bold uppercase ${isOpen ? job.headerColor : ''}`}>{job.name}</span>
                    ...
                    {isOpen && (
                      <div className="px-2 pb-2 pt-1 space-y-2 bg-gray-900/50">
                        {job.algorithms.map(alg => (
                          <div key={alg.type} draggable onDragStart={(e) => onDragStart(e, alg.type)} className={`${alg.color} group p-2 rounded-md cursor-grab hover:translate-x-1 transition border border-white/10`}>
                            <span className="text-xs text-white w-full text-center">{alg.label}</span>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            {activeTab === 'templates' && templateJobs.map(job => {
                  ...
                  return (
                    <div key={job.name} className="rounded-lg overflow-hidden border border-gray-800/50 bg-gray-800/30 mb-2">
                      <div className={`flex items-center justify-between p-2.5 cursor-pointer ${isOpen ? 'bg-gray-800 text-gray-200' : 'hover:bg-gray-800 text-gray-400'}`} onClick={() => toggleTemplateGroup(job.name)}>
                      ...
                      {isOpen && (
                        <div className="px-2 pb-2 pt-1 space-y-2 bg-gray-900/50">
                          {job.templates.map((t, idx) => {
                            ...
                            return (
                              <div key={idx} onClick={() => onLoadTemplate?.(t)} onContextMenu={(e) => openContextMenu(e, t)} className={`group p-2 rounded-lg border border-gray-700 bg-gray-800/50 hover:bg-gray-800 ${style.border} cursor-pointer transition relative overflow-hidden`}>
                                <div className={`absolute left-0 top-0 bottom-0 w-1 ${style.stripe} opacity-0 group-hover:opacity-100 transition`} />
                                <h3 className={`text-xs font-bold ${style.text} ${style.hoverText} mb-1`}>TEMPLATES: {t.name}</h3>
      ...
      {contextMenu.visible && (
        <div className="fixed z-[9999] bg-gray-800 border border-gray-700 text-sm rounded-md shadow-xl py-1 w-48" style={{ top: contextMenu.y, left: contextMenu.x }}>
          ...
          <button className="w-full text-left px-3 py-2 hover:bg-gray-700 text-white" onClick={() => { setAboutModal({ visible: true, template: contextMenu.template }); setContextMenu((c) => ({ ...c, visible: false })); }}>🧩 About This Algorithm</button>
        </div>
      )}
