...
def img_pair_rot(tmp_path):
    """ภาพ A และ A ที่หมุน 45° (เพื่อให้มี inliers พอสมควร)"""
    a = tmp_path / "a.jpg"
    b = tmp_path / "b.jpg"
...
def _ensure_feat_orb(tmp_path: Path, img_path: str, WTA_K=2, nfeatures=800, **params) -> str:
    """รัน ORB แล้วคืน json_path (ORB adapter เขียนลง ./outputs/...)"""
...
def test_schema_and_files_sift_flann(tmp_path, img_pair_rot):
    ...
    # ช่องหลัก ๆ ควรอยู่ครบ
    for k in ["matching_tool", "flann_parameters_used", "input_features_details",
              "matching_statistics"]:
        assert k in out
...
def test_schema_and_files_orb_flann(tmp_path, img_pair_rot):
    ...
    out = flann_run(ja, jb, out_root=str(tmp_path))
    assert os.path.exists(out["json_path"])
    assert out["flann_parameters_used"]["index_selected"] in ("LSH", "KD_TREE")  # AUTO เลือกตาม tool


# ============================================================
# 2) Index selection & compatibility
# ============================================================
...
def test_force_kdtree_on_sift_is_respected(tmp_path, img_pair_rot):
    ...
    assert p["index_selected_reason"] == "override_respected"
    assert p["index_params"]["algorithm"] == 1
...
def test_invalid_index_mode_for_sift_raises(tmp_path, img_pair_rot):
    ...
    with pytest.raises(ValueError):
        flann_run(ja, jb, out_root=str(tmp_path), index_mode="LSH")  # SIFT ห้าม LSH


def test_invalid_index_mode_for_orb_raises(tmp_path, img_pair_rot):
    a, b = img_pair_rot
    ja = _ensure_feat_orb(tmp_path, a, WTA_K=3)
    jb = _ensure_feat_orb(tmp_path, b, WTA_K=4)
    with pytest.raises(ValueError):
        flann_run(ja, jb, out_root=str(tmp_path), index_mode="KD_TREE")  # ORB ห้าม KD_TREE
...
# ============================================================
# 3) Echo / humanizer / search params
# ============================================================
...
def test_lsh_params_echo_for_orb(tmp_path, img_pair_rot):
    ...
    ip = p["index_params"]
    assert ip["algorithm"] == 6
    assert ip["table_number"] == 10
    assert ip["key_size"] == 14
    assert ip["multi_probe_level"] == 3
    assert p["search_params"]["checks"] == 42
    name = p["index_name"]
    assert "LSH" in name and "table=10" in name and "key=14" in name and "multi=3" in name


# ============================================================
# 4) Lowe's ratio defaults / overrides
# ============================================================
...
def test_invalid_lowe_ratio_raises(tmp_path, img_pair_rot):
    ...
    jb = _ensure_feat_sift(tmp_path, b)
    with pytest.raises(ValueError):
        flann_run(ja, jb, out_root=str(tmp_path), lowe_ratio=0.0)
    with pytest.raises(ValueError):
        flann_run(ja, jb, out_root=str(tmp_path), lowe_ratio=1.0)
    with pytest.raises(ValueError):
        flann_run(ja, jb, out_root=str(tmp_path), lowe_ratio=-0.25)


# ============================================================
# 5) RANSAC / draw modes / echo
# ============================================================
...
def test_ransac_thresh_echo_and_invalid(tmp_path, img_pair_rot):
    ...
    with pytest.raises(ValueError):
        flann_run(ja, jb, out_root=str(tmp_path), ransac_thresh=0.0)
    with pytest.raises(ValueError):
...
def test_draw_mode_invalid_is_clamped_to_good(tmp_path, img_pair_rot):
    ...
    out = flann_run(ja, jb, out_root=str(tmp_path), draw_mode="WEIRD_MODE")  # ไม่รองรับ
...
def test_max_draw_zero_is_accepted_and_echoed(tmp_path, img_pair_rot):
    ...
    out = flann_run(ja, jb, out_root=str(tmp_path), max_draw=0)  # 0 = วาดทั้งหมด
    assert out["flann_parameters_used"]["max_draw"] == 0


# ============================================================
# 6) Homography reasons
# ============================================================

def test_homography_reason_not_enough_good_matches(tmp_path, img_pair_diff):
    """
    ใช้ภาพต่างกันมาก + ratio เข้ม เพื่อให้ good_matches < 4
    """
    a, b = img_pair_diff
...
def test_homography_failure_reason_exposed(tmp_path):
    """
    บังคับ findHomography คืน None เพื่อตรวจ reason = 'findHomography_failed'
    เงื่อนไข: ต้องมี good_matches >= 4 ก่อน → ใช้ภาพเดียวกัน
    """
    img = tmp_path / "z.jpg"
    h, w = 320, 320
...
# ============================================================
# 7) Sanity: statistics relationship
# ============================================================
...
def test_inliers_not_more_than_good_matches(tmp_path, img_pair_rot):
    ...
    ja = _ensure_feat_orb(tmp_path, a, WTA_K=2, nfeatures=800)
    jb = _ensure_feat_orb(tmp_path, b, WTA_K=2, nfeatures=800)
    out = flann_run(ja, jb, out_root=str(tmp_path))  # AUTO → LSH
